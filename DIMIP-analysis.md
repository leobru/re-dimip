...
| 02040 | `Э67 D02400` — install **debug/abort handler**; info word `D02400 = A(ГЛЦИКЛ)` → after abort or MKP stack overflow, resume the main loop. |
...
`boom.txt` confirms the Э67 stack-overflow path. The recursive macro exhausts
the MKP stack, the `МЕТКИ` marker-scan routine at `03540..03551` selects the
inline text `ПЕРЕП. СТЕК`, and returns through `02040` to `ГЛЦИКЛ`. After the repeated
failed macro command, normal task termination goes through `03523..03525` to
`03507:*62`.
...
(`<ХХХ[=ARG]...`); all other lines are copied to the temp area. **Working session**
...
- **Entry** (`03224`): a directive that fails the `ТАБДИР` scan is treated as `<ИМЯФ>` —
  ...
  (`VAR00`) := default directive char `<` + МП char `%`; then catalog search + line loop.
...
- **`SIТ` dispatch**: `<SIТ=DIR=MACR=CODE` writes a handler descriptor into the table at
  ...
  Verified by `sit.txt`: `<SIТ=СLО=SAY=3` catches `<СLО=1` and prints `SAY` before the
  following `<МЕS`.
- **Macro variables**: `МПn` = 4 words at cells `4n+1..4n+4` (`VAR00`=1–4, `МП10`=51₈–54₈,
  verified); `АДРМП 04102` resolves n→М4. Text УПП, `0377` terminator. КОТ = byte of
  `VAR00` word 1 (`УСТКОТ G04333`).
...
- **`СОN`** (`КОМСОN 04421`) is an undocumented **substring search**:
  `<СОN=VAR=TEXT` searches the text value of `VAR` for literal `TEXT`; on success it writes
  the **1-based** first-match position into `МП01` (`VAR01`), and on failure leaves `МП01`
...
|---------|------|--------------------|
| `0001`–`0004` | `МП00` | `VAR00`, МКП variable 0: word 1 holds the МКП directive char `<` (byte 1), macro-call level (byte masked by `D02441`, decremented by `КОНМАК`), КОТ (written by `УСТКОТ G04333`), МП char `%` (byte 6). Initialized at МКП entry (`03234`). |
| `0005`– | `МП01` | `VAR01`: `<INF` puts the current time here; `<ОРЕ` (no-channel form) the file start zone; **`ДИРФ` stores the formed task number here** (`зп 5` — cf. §6.2.6, ДИМИП passes data to macros via VAR01). |
| `0011`– | `МП02` | `VAR02`: `<INF` — monitor шифр; `<ОРЕ` — file length. |
| `0015`– | `МП03` | `VAR03`: `<INF` — date (2 words, `0015`/`0016`); `<ОРЕ` — file type. |
...
| `1720` | `НКАН` | subtask channel number `<НК>` — `ДИСПАТ` packs `АРГ1`'s value into the top byte (`сбр D02442; сда 64+36`), i.e. the `Э62` channel-argument field; read by every ПЗ op (`Б`/`ЗП`/`ПП`/`ВЫБ`, `ПЗНОВ`/`ПЗСТОП`, `ПЗКАТ`→`Э50 151`). Was `ОПВЫВ+3`. |
...
`ДИРФ` stores the formed task number in `VAR01` (`03434: зп 5`) and tests
`АРГ3+26`:
...
The МОНИТОР extracode manual documents only `АИСП = 7701` for the formation family (§5.3.67);
...
**МКП async layer (not the monitor path).** During `<GET` terminal input, byte 5 of `VAR00`
...
- **Field separator**: the symbolic-field path uses **GOST code `025`**, i.e. `=`.
  After `<RЕА=VAR=НК=ПОЛЕ` finds the field index (`G04264`), `G04273`/`G04116` fetches
...
Channel errors do **not** abort the macro: `G04333` (04333) records the code in a VAR00
